...
if __name__ == '__main__':
    ...
    for iter in range(args.epochs):
        if args.enc:
            ...
            w_sum = FedWeightAvg(w_locals)
            w_avg = OrderedDict()
            for name, param_list in w_sum.items():
                # 将参数列表转换为 numpy 数组
                param_array = np.array(param_list)
                # 将参数除以 10
                param_array /= float(m)
                # 找到原始模型中的对应参数以获取正确的形状和设备
                original_param = net_glob.state_dict()[name]
                # 将 numpy 数组转换为 PyTorch Tensor，并调整类型和设备
                param_tensor = torch.tensor(param_array, dtype=original_param.dtype, device=original_param.device)
                # 确保形状正确
                param_tensor = param_tensor.view(original_param.shape)
                # 更新处理后的参数字典
                w_avg[name] = param_tensor
            #     params = [param / float(m) for param in params]
            #     tensor_params = torch.tensor(params)
            #     w_glob[layer] = tensor_params
            # w_glob.to(args.device)
            w_glob = w_avg
    ...
    filename = './log/fedavg_{}_{}_{}_C{}_acc.txt'.format(args.dataset, args.model, args.epochs, args.frac)
